...
bool waitFall = false;
bool waitTeleport = false;

// Return true si tombé dans le trou
bool Fallen_Spikes(Rectangle *rect_piques, Rectangle *rect_solin) {
	return CheckCollisionRecs(*rect_solin, *rect_piques);
}

bool Dead(Player *player_Struct) {
	return player_Struct->health_point == 0;
}
...
bool Use_Teleport(Rectangle *rect_solin, Rectangle *rect_tp) {
	return CheckCollisionRecs(*rect_solin, *rect_tp );
}
...
void LevelTwoCheck_Event(Player *player_Struct, PhysicsBody *body, Rectangle *rect_piques, Rectangle *rect_tp, Rectangle *rect_solin, Rectangle *rect_lean1, Rectangle *rect_lean2, Rectangle *rect_lean3) {
	...
	if ( Fallen_Spikes(rect_piques, rect_solin )  || !canMove ) {

		if(!waitFall) { //Tombé mais pas encore attendu dans les pics
			waitFall = true;
			player_Struct->health_point -= 1 ;
			imgPlayer = solinDead;
			canMove = false;
		}
		else { // Après être tombé et avoir attendu
	...
	if ( Use_Teleport(rect_solin, rect_tp ) ) {
		if( player_Struct->nb_lean == 3) {
			if ( !waitTeleport ) {	
